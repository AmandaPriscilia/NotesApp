class NotesData {
  ...
  static getAllNotes() {
    ...
    return savedNotes ? JSON.parse(savedNotes) : this.initDefaultNotes();
  }

  static initDefaultNotes() {
    this.saveNotes(this.DEFAULT_NOTES);
  ...
  static addNote(note) {
    ...
    const newNote = this.createNewNote(note);
    notes.push(newNote);
  ...
  static archiveNote(id) {
    const notes = this.getAllNotes().map((note) =>
      note.id === id ? { ...note, archived: true } : note,
    );
    this.saveNotes(notes);
  }
  static getDefaultNotes() {
    return this.DEFAULT_NOTES;
  }
}
